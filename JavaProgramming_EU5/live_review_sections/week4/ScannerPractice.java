...
public class ScannerPractice {
	...
	public static void main(String[] args) {
		...
		for (int i = 1; i <= 5; i++) {
			...
			if (number > max) { // compare the inputs with each other, at the end the maximum number will be
								// assigne to max
				max = number; // if the input is greater than previous then it will be assigned to max
			}

			if (number < min) { // compare the inputs with each other, at the end the minimum number will be
								// assigne to min
